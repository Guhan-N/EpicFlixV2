(() => {
  // js/api.js
  var TMDB_API_KEY = "44983101bc2e8821e3a20c61a9df7a4a";
  var TMDB_API_BASE_URL = "https://api.themoviedb.org/3";
  var BACKDROP_BASE_URL = "https://image.tmdb.org/t/p/original";
  ...
  var StorageManager = {
    ...
    history: {
      add(item) {
        ...
        const normalizedItem = {
          ...
          poster_path: item.poster_path || null,
          watchedAt: item.watchedAt || (/* @__PURE__ */ new Date()).toISOString(),
    ...
    rating: {
      set(type, id, value) {
        const ratings2 = readStorage(STORAGE_KEYS.ratings, {});
        ratings2[`${type}-${id}`] = value;
        writeStorage(STORAGE_KEYS.ratings, ratings2);
        return ratings2;
      },
      get(type, id) {
        const ratings2 = readStorage(STORAGE_KEYS.ratings, {});
        return ratings2[`${type}-${id}`] || 0;
      }
    },
    theme: {
      save(theme) {
  ...
  function createAlertElement(type, title, message) {
    ...
    <button class="alert-close" onclick="this.parentElement.remove()">\xD7</button>
  `;
    return alertElement;
  }
  function showAlert(type, title, message, duration = 5e3) {
  ...
  function removeFromWatchlist(contentId, contentType2) {
    StorageManager.watchlist.remove(contentId, contentType2);
  ...
  // js/history.js
  function addToHistory(item, type, progress = 0, season = 1, episode = 1) {
  ...
  // js/theme.js
  function applySavedTheme(themeToggle2) {
    const savedTheme = StorageManager.theme.load() || "dark";
    document.body.setAttribute("data-theme", savedTheme);
    if (themeToggle2) {
      themeToggle2.textContent = savedTheme === "dark" ? "" : "";
    }
    return savedTheme;
  }
  function toggleTheme(themeToggle2) {
    const currentTheme = document.body.getAttribute("data-theme") || "dark";
    const newTheme = currentTheme === "dark" ? "light" : "dark";
    document.body.setAttribute("data-theme", newTheme);
    StorageManager.theme.save(newTheme);
    if (themeToggle2) {
      themeToggle2.textContent = newTheme === "dark" ? "" : "";
    }
    return newTheme;
  }
  ...
  var mobileGenreSelect = document.getElementById("mobile-genre-select");
  var contentType = document.getElementById("content-type");
  var mobileContentType = document.getElementById("mobile-content-type");
  var mobileMenuToggle = document.getElementById("mobile-menu-toggle");
  var mobileSidebarMenu = document.getElementById("mobile-sidebar-menu");
  var closeMobileMenuButton = document.querySelector(".close-mobile-menu");
  ...
  function createContentCard(item, type = "movie") {
    ...
    <div class="movie-poster-container">
      ...
      <div class="card-actions">
        <button class="action-btn watchlist-btn" onclick="window.addToWatchlistFromApp(${JSON.stringify(item).replace(/"/g, "&quot;")}, '${type}')" title="Add to Watchlist">\u{1F4CB}</button>
      </div>
    </div>
  ...
  async function showDetails(id, type) {
        ...
        <div class="details-content">
          ...
          <div class="details-info">
            ...
            <div class="content-actions">
              <button class="action-button watchlist-button" onclick="window.addToWatchlistFromApp(${JSON.stringify(details).replace(/"/g, "&quot;")}, '${type}')">\u{1F4CB} Add to Watchlist</button>
              <button class="action-button" id="share-btn"><span>\u{1F517}</span> Share</button>
            </div>
          </div>
        </div>
      </div>
        ...
        ${trailer || initialVidsrcUrl ? `
          <section class="video-player-section">
            <div class="video-controls">
              <button class="video-btn ${!trailer ? "disabled" : ""}" id="watch-trailer" ${!trailer ? "disabled" : ""}>\u{1F3AC} Watch Trailer</button>
    ...
    const updateVideoSource = (src) => {
      const videoPlayer2 = document.getElementById("video-player");
      if (videoPlayer2 && src) {
    ...
    document.querySelectorAll(".star").forEach((star) => {
      star.addEventListener("click", () => {
        const rating2 = parseInt(star.dataset.rating);
        setRating(type, id, rating2);
        document.querySelectorAll(".star").forEach((s, i) => s.classList.toggle("active", i < rating2));
      });
    });
    ...
    submitButton?.addEventListener("click", () => {
      ...
      if (text) {
        const comment = { text, date: (/* @__PURE__ */ new Date()).toISOString() };
  ...
  async function displayContent(page) {
    ...
    switch (page) {
      case "home": {
        ...
        new Splide(".splide", { type: "fade", rewind: true, autoplay: true, interval: 5e3 }).mount();
